...
/**
 *   	\file       calibracion_list.php
 *		\ingroup    calibracion
 *		\brief      List page for calibracion
 */
...
require_once __DIR__.'/class/calibracion.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('calibracion', 'calibracion', 'write');
	$permissiontodelete = $user->hasRight('calibracion', 'calibracion', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("calibracion")) {
	accessforbidden('Module calibracion not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Calibracion';
	$objectlabel = 'Calibracion';
	$uploaddir = $conf->calibracion->dir_output;
...
$title = $langs->trans("Calibracions");
//$help_url = "EN:Module_Calibracion|FR:Module_Calibracion_FR|ES:Módulo_Calibracion";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/calibracion/calibracion_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-calibracion page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/calibracion/calibracion_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCalibracionRef";
$modelmail = "calibracion";
$objecttmp = new Calibracion($db);
